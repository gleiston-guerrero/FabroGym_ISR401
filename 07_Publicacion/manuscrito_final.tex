...
Every numerical statement in this Results section corresponds to a CSV output generated from the versioned analysis package. The single entry point regenerates multimedia summaries, questionnaire frequencies and bootstrap intervals, saturation tables, and member-checking summaries. This design follows reproducible-research principles and reduces the risk of manuscript-only numbers \cite{sandve2013,peng2011,wilkinson2016,smith2016}.
...
The corrective pass was treated as an auditable closure exercise rather than as an opportunity to rewrite the historical record. Historical authorship remains preserved in Git, the ERS/SRS, and the published research artifacts. In the absence of a documented instructor authorization reducing the evaluated group, the assessment scope remains the five project members: Alvia Villegas Erick Adalberto, Mera Arias Erick Jhair, Mora Duarte Alex Jos{\'e}, Ponce Rivera Mery Helenmey, and Vaca Romero David Octavio. Recent corrective work is documented mainly for Mera, Mora, and Ponce; that fact does not redefine the evaluated team. Missing individual evidence is not fabricated or reassigned. The canonical scope statement is \path{10_Autoria/EQUIPO_EXAMEN_FINAL.md}.
...
\midrule
\S15 & A prior corrective interpretation reduced the examination scope to three members without documented instructor authorization; the photographic inventory also required normalization. & The assessment scope is restored to the five project members. A2 currently documents 13 captures for Mera, 10 for Mora, 19 for Ponce, and no sufficient current-cut captures located for Alvia or Vaca; no evidence is fabricated. The EXIF inventory contains 11 real records (2 team, 4 environment, 5 questionnaire). & Repository-side correction completed; Alvia/Vaca A2 evidence remains an external factual pending item unless authentic evidence is supplied. \\
\S7 & Four public consent PDFs did not themselves evidence a scanned signature because the prior representation was Word-generated. & ENTR-02, ENTR-03, ENTR-04, and ENTR-06 were replaced by rasterized, redacted scans of the signed forms. The four public PDFs yield zero extracted words under \texttt{pdftotext}; metadata and public/restricted controls are documented in \path{08_Etica/CONTROL_CONSENTIMIENTOS_FINAL.md}. & Mera Arias / \texttt{Emeraxs} (repository correction record \texttt{a68594a}). \\
...
\S16 & The report did not consolidate the corrected effect table, survey limitation, consent correction, closure baseline, and a named correction retrospective in one auditable artifact. & This source and its generated PDF now consolidate those elements, including Table~\ref{tab:effect-profile}, the validity limitations, the consent correction above, this retrospective, and the distinction between the published baseline and a later candidate closure. The report is regenerated from \path{07_Publicacion/manuscrito_final.tex} with \path{07_Publicacion/compilar_manuscrito.py}. & Cross-review of the corrective report; this responsibility statement does not redefine the evaluated team. \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Declared closure baseline.}
The canonical repository for the final examination delivery is \url{https://github.com/gleiston-guerrero/FabroGym_ISR401}. The annotated tag \texttt{v2.0.2-final} is an already-published historical baseline and is not moved or overwritten. Corrections after that baseline remain PRE-TAG. If all factual blockers are closed and the integrity manifests are regenerated only after the final content change, the next candidate closure identifier is \texttt{v2.0.3-final}; it is not declared current in this report.
...
The canonical examination repository is \url{https://github.com/gleiston-guerrero/FabroGym_ISR401}. The tag \texttt{v2.0.2-final} is retained as the latest published historical baseline; any subsequent terminal tag is created only after the remaining factual blockers and terminal integrity checks are closed. The preregistration is publicly identified at OSF (DOI: \href{https://doi.org/10.17605/OSF.IO/62YSC}{10.17605/OSF.IO/62YSC}). The replication package was published on Zenodo as version 2.0.0 (DOI: \href{https://doi.org/10.5281/zenodo.22237884}{10.5281/zenodo.22237884}). The later identifier normalization in the repository is not claimed to be byte-identical to that published snapshot. Public materials include anonymized transcripts, the anonymized questionnaire, requirement corpora, traceability, processed results, and reproducible scripts. Identifiable audio/video and original consent materials remain restricted and are not part of the open dataset. Data publication follows FAIR and software-citation principles \cite{wilkinson2016,smith2016,dicosmo2020}.
