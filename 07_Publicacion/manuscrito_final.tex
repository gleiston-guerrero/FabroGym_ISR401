...
The canonical repository for the final examination delivery is \url{https://github.com/gleiston-guerrero/FabroGym_ISR401}. The annotated tags \texttt{v2.0.0-final} through \texttt{v2.0.4-final} are retained as published historical baselines and are not moved or overwritten. Tag \texttt{v2.0.4-final} (\texttt{dc5a228}) identifies the line evaluated in the 17 September 2026 report. Corrections after that report will be closed with a new annotated tag, expected by sequence to be \texttt{v2.0.5-final}, only after the content is frozen, a new signed pre-verification document is incorporated, and both SHA-256 manifests verify successfully from a clean clone.
