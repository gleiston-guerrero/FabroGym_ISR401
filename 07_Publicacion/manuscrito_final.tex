...
The corrective pass is documented as an auditable sequence rather than as a rewrite of the historical record. The report issued on 17 September 2026 evaluated tag \texttt{v2.0.4-final} (\texttt{dc5a228}) and found that the scientific manuscript itself was compliant, while the use-case text, root-manifest reproducibility, A2 screenshots, closure documentation, and retrospective still required correction. Historical authorship remains preserved for all five contributors. Technical work after the guide is attributed only to the people who actually versioned it, while the 17 September commits by Alvia and Vaca are classified specifically as personal deposits of historical A2 evidence requested by the instructor.

\begin{table}[H]
\caption{Corrections performed after the 17 September failed-exam report and documented responsibility.}\label{tab:retrospective}
\centering\scriptsize
\begin{tabularx}{\textwidth}{@{}p{1.0cm}X X p{3.3cm}@{}}
\toprule
Item & Review finding & Correction and current evidence & Documented responsibility \\
\midrule
\S4 & Seven use cases had exception conditions that contradicted their own preconditions; several alternative flows were not valid variants and no alternative flow declared its branching point. & The seven contradictions were removed, the questioned alternatives were rewritten, all 19 alternative flows now state their opening step and return/termination point, and the traceability matrix remains synchronized with 57 flow identifiers. & Mera / \texttt{Emeraxs}: ERS correction (\texttt{1dd541b}); Ponce / \texttt{Mery-003}: matrix synchronization (\texttt{0d42923}). \\
\S12 & The root manifest did not verify on a clean clone because of CRLF/LF differences, and the two \texttt{run\_all.py} files differed. & Both orchestrators are synchronized, relevant text outputs are forced to LF, and the 38 result files in \texttt{07\_Datos/resultados/} and \texttt{06\_Experimento/resultados/} remain byte-identical. Terminal manifests are intentionally deferred until all content is frozen. & Mera: \texttt{53d17a0}, \texttt{318f4eb}; Ponce: \texttt{6a68f3c}, \texttt{6244ea8}. \\
\S15 & Six screenshots attributed to Alvia and Vaca had been taken from another member's GitHub session. & The six observed screenshots were removed. Vaca personally deposited three screenshots of his historical commits (\texttt{2ecbe55}); Alvia personally deposited the valid A2 set in the canonical folder (\texttt{47cac43}). The earlier misplaced Alvia upload is treated as superseded and duplicate root copies were removed. & Vaca / \texttt{David-Bs1}: \texttt{2ecbe55}; Alvia / \texttt{Erick-Alvia}: \texttt{47cac43}; Mera: removal, verification and cleanup (\texttt{0bf5014}, \texttt{bf82034}, \texttt{ce89a2b}); Ponce: documentary consolidation (\texttt{fccdb9d}). \\
Obs. WALK & The MP4 for WALK-TEC-01 stores an internal \texttt{creation\_time} of 05/08/2026, earlier than the documented session date of 12/08/2026. & The internal container timestamp is retained and not rewritten. The session date remains 12/08/2026 based on the field note, technical sheet, SHA-256 identity, duration and prior traceability; the distinction is documented explicitly. & Mera: technical metadata verification (\texttt{d995cdb}); Ponce: documentary closure (\texttt{ab8d897}). \\
Obs. commit & Commit \texttt{fef33d8} stated that it added a script, but the versioned change only modified \texttt{checksums.sha256}. & The historical commit is not rewritten. \path{04_Trazabilidad/ACLARACION_COMMIT_FEF33D8.md} records the mismatch and prevents the message from being used as evidence that a script was added. & Documentary clarification in the post-report closure. \\
\S16 & The retrospective had become stale: it omitted a current ``who did what'' section, contained statements that no longer matched the repository, and did not identify who took the A2 screenshots. & The canonical retrospective is dated 17/09/2026, records the post-report work by person and commit, distinguishes historical A2 deposits from new technical work, and updates the closure baseline. This manuscript is recompiled only for documentary consistency; the accepted numerical results, research questions and scientific conclusions are unchanged. & Mera and Ponce: post-report technical/documentary closure; Alvia and Vaca: personal historical A2 deposits only; Mora: no new post-report technical work is claimed. \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Declared closure baseline.}
The canonical repository for the final examination delivery is \url{https://github.com/gleiston-guerrero/FabroGym_ISR401}. The annotated tags \texttt{v2.0.0-final} through \texttt{v2.0.4-final} are retained as published historical baselines and are not moved or overwritten. Tag \texttt{v2.0.4-final} (\texttt{dc5a228}) identifies the line evaluated in the 17 September 2026 report. Corrections after that report use \texttt{v2.0.5-final} as the terminal closure identifier. The tag is created and published only after the content is frozen, a new signed pre-verification document is incorporated, and both SHA-256 manifests verify successfully from a clean clone.

\paragraph{Integrity rule for the retrospective.}
The retrospective records only evidence that exists in the repository or is generated reproducibly from it. It does not create missing screenshots, signatures, participant responses, statistical observations, or historical contributions. The personal A2 deposits made by Alvia and Vaca on 17 September document older commits and are not treated as new technical contributions to the corrected rubric items. Limitations that cannot be repaired retroactively remain explicitly declared.

\section*{Data and materials availability}
The canonical examination repository is \url{https://github.com/gleiston-guerrero/FabroGym_ISR401}. The tags \texttt{v2.0.0-final} through \texttt{v2.0.4-final} are retained as published historical references, with \texttt{v2.0.4-final} (\texttt{dc5a228}) marking the line evaluated on 17 September 2026. The corrected terminal closure is identified as \texttt{v2.0.5-final}; the tag is created and published only after the content is frozen, a new signed pre-verification document is incorporated, and the root and \texttt{07\_Datos} SHA-256 manifests verify successfully from a clean clone. The preregistration is publicly identified at OSF (DOI: \href{https://doi.org/10.17605/OSF.IO/62YSC}{10.17605/OSF.IO/62YSC}). The replication package was published on Zenodo as version 2.0.0 (DOI: \href{https://doi.org/10.5281/zenodo.22237884}{10.5281/zenodo.22237884}). The later repository corrections are not claimed to be byte-identical to that published snapshot. Public materials include anonymized transcripts, the anonymized questionnaire, requirement corpora, traceability, processed results, and reproducible scripts. Identifiable audio/video and original consent materials remain restricted and are not part of the open dataset. Data publication follows FAIR and software-citation principles \cite{wilkinson2016,smith2016,dicosmo2020}.
