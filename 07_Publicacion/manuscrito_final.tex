...
The corrective pass was treated as an auditable closure exercise rather than as an opportunity to rewrite the historical record. Historical authorship remains preserved in Git, the ERS/SRS, and the published research artifacts. The team that performs the current examination closure is \textbf{Mera Arias Erick Jhair, Mora Duarte Alex Jos{\'e}, and Ponce Rivera Mery Helenmey}; Alvia Villegas Erick Adalberto and Vaca Romero David Octavio remain historical contributors and are not assigned evidence created during this closure. The canonical scope statement is \path{10_Autoria/EQUIPO_EXAMEN_FINAL.md}.
...
\midrule
\S15 & Closure authorship evidence could be misread as requiring current-exam evidence from historical contributors; photographic inventory also required normalization. & The current examination team is delimited to Mera, Mora, and Ponce without deleting historical authorship. A2 attribution, team photographs, five questionnaire-application photographs, and EXIF inventory are documented in \path{10_Autoria/}. & Ponce Rivera / \texttt{Mery-003} (repository correction record \texttt{efa727f}). \\
...
\S16 & The report did not consolidate the corrected effect table, survey limitation, consent correction, closure baseline, and a named correction retrospective in one auditable artifact. & This source and its generated PDF now consolidate those elements, including Table~\ref{tab:effect-profile}, the validity limitations, the consent correction above, this retrospective, and the declared closure identifier. The report is regenerated from \path{07_Publicacion/manuscrito_final.tex} with \path{07_Publicacion/compilar_manuscrito.py}. & Mera, Mora, and Ponce (cross-review of the final examination report). \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Declared closure baseline.}
The canonical repository for the final examination delivery is \url{https://github.com/gleiston-guerrero/FabroGym_ISR401}. The closure identifier declared as vigente for this delivery is the annotated tag \texttt{v2.0.2-final}. No later tag is declared current in this report. The prescribed closure sequence regenerates the integrity manifests over this frozen content immediately before that annotated tag is placed on the terminal commit; consequently, this manuscript is not edited after the present closure section is frozen.
