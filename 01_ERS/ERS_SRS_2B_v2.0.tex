...
Fecha de corte documental & 15 de septiembre de 2026 \\\hline
...
\end{tabularx}
\vspace{0.35cm}
{\normalsize\bfseries Composición vigente para el examen suspenso\par}
\vspace{0.1cm}
\small
\begin{tabularx}{\textwidth}{|X|X|p{4.15cm}|}\hline
\textbf{Integrante} & \textbf{Condición en esta evaluación} & \textbf{ORCID}\\\hline
Mera Arias Erick Jhair & Evaluado en examen suspenso & 0009-0001-0068-1796\\\hline
Ponce Rivera Mery Helenmey & Evaluada en examen suspenso & 0009-0006-6041-9198\\\hline
Mora Duarte Alex José & Apoyo no evaluado; autoría propia preservada & 0009-0000-2494-2842\\\hline
\end{tabularx}
\vspace{0.12cm}
\footnotesize \textit{Participación histórica fuera del examen suspenso: Alvia Villegas Erick Adalberto y Vaca Romero David Octavio. Sus aportes previos se conservan en Git y no se reasignan. La composición vigente se formaliza en 04\_Trazabilidad/solicitud\_cambio\_composicion\_equipo.pdf.}
\vfill
...
\begin{importantbox}
\textbf{Convención de trazabilidad de flujos.} \texttt{FP} identifica el flujo principal, \texttt{FA} un flujo alternativo válido del negocio y \texttt{EX} una excepción que impide o desvía la operación. Las condiciones y respuestas aquí descritas refinan el comportamiento de los requisitos Must existentes; no crean RF adicionales.
\end{importantbox}
...
\begin{itemize}
\item El usuario dispone de una cuenta individual habilitada para demostración.
...
\textbf{Flujo alternativo \texttt{CU-01-FA-01}.} \textit{Condición de disparo:} El usuario autenticado solicita cerrar su sesión.
\begin{enumerate}[label=\alph*.]
\item El sistema invalida la sesión activa.
\item El sistema registra el cierre de sesión y retorna a la pantalla de acceso.
\end{enumerate}
...
\item Con cierre o rechazo, no queda una sesión activa creada por el intento.
\end{itemize}
...
\textbf{Flujo alternativo \texttt{CU-02-FA-01}.} \textit{Condición de disparo:} El rol permite consulta pero no modificación sobre el recurso solicitado.
\begin{enumerate}[label=\alph*.]
...
\item Se dispone de código interno, nombre de uso y al menos un medio de contacto permitido.
\end{itemize}
...
\textbf{Flujo alternativo \texttt{CU-03-FA-01}.} \textit{Condición de disparo:} No se proporcionan datos opcionales distintos del conjunto mínimo.
\begin{enumerate}[label=\alph*.]
\item El sistema permite continuar con código, nombre de uso y contacto.
\item El cliente se registra únicamente con los datos mínimos autorizados.
\end{enumerate}
...
\textbf{Flujo alternativo \texttt{CU-04-FA-01}.} \textit{Condición de disparo:} La búsqueda devuelve más de una coincidencia válida.
\begin{enumerate}[label=\alph*.]
...
\textbf{Flujo alternativo \texttt{CU-05-FA-01}.} \textit{Condición de disparo:} Durante una intención de alta se detecta coincidencia con un cliente existente.
\begin{enumerate}[label=\alph*.]
\item El sistema advierte la coincidencia antes de crear otro registro.
\item El usuario selecciona el registro existente y continúa por actualización o reactivación.
\end{enumerate}
...
\textbf{Flujo alternativo \texttt{CU-06-FA-01}.} \textit{Condición de disparo:} El administrador decide desactivar un plan o promoción existente sin eliminarlo.
\begin{enumerate}[label=\alph*.]
...
\item El plan seleccionado está vigente.
\item Existe un pago confirmado asociado a la operación.
\end{itemize}
...
\textbf{Flujo alternativo \texttt{CU-07-FA-01}.} \textit{Condición de disparo:} El cliente ya posee una membresía y la operación corresponde a renovación.
\begin{enumerate}[label=\alph*.]
\item El sistema identifica la membresía existente.
\item El sistema registra la renovación conservando el historial y recalculando las fechas con el plan vigente.
\end{enumerate}
...
\textbf{Precondiciones.}
\begin{itemize}
\item El usuario mantiene una sesión autorizada.
\item El cliente puede ser localizado.
\end{itemize}
...
\textbf{Precondiciones.}
\begin{itemize}
\item El usuario está autorizado.
\item Existe una fecha de corte válida para la consulta.
\end{itemize}
...
\textbf{Flujo alternativo \texttt{CU-09-FA-01}.} \textit{Condición de disparo:} No existen membresías vencidas ni próximas a vencer en la ventana definida.
\begin{enumerate}[label=\alph*.]
...
\textbf{Precondiciones.}
\begin{itemize}
\item El usuario está autorizado para registrar pagos.
\item La operación dispone de monto, concepto, medio, fecha y la referencia aplicable.
\end{itemize}
...
\textbf{Flujo alternativo \texttt{CU-10-FA-01}.} \textit{Condición de disparo:} El medio de pago no requiere una referencia externa, como en un pago en efectivo.
\begin{enumerate}[label=\alph*.]
...
\textbf{Flujo alternativo \texttt{CU-11-FA-01}.} \textit{Condición de disparo:} La asistencia requiere una excepción operativa autorizada.
\begin{enumerate}[label=\alph*.]
...
\textbf{Flujo alternativo \texttt{CU-12-FA-01}.} \textit{Condición de disparo:} La combinación de filtros no devuelve eventos.
\begin{enumerate}[label=\alph*.]
...
\textbf{Flujo alternativo \texttt{CU-13-FA-01}.} \textit{Condición de disparo:} El producto ya existe y el administrador solicita modificar sus datos permitidos o su estado.
\begin{enumerate}[label=\alph*.]
...
\item El usuario tiene permiso para registrar movimientos de stock.
\end{itemize}

\textbf{Flujo principal \texttt{CU-14-FP}.}
\begin{enumerate}
\item El usuario selecciona el producto.
\item Indica el tipo de movimiento, cantidad y motivo.
\item El sistema identifica al responsable desde la sesión.
\item El sistema calcula el saldo resultante.
\item El sistema valida que el saldo sea consistente.
...
\textbf{Flujo alternativo \texttt{CU-14-FA-01}.} \textit{Condición de disparo:} El movimiento corresponde a un ajuste que reduce existencias, pero el saldo resultante permanece no negativo.
\begin{enumerate}[label=\alph*.]
\item El sistema calcula el nuevo saldo.
\item El ajuste se registra con su motivo y responsable y continúa el flujo normal.
\end{enumerate}
...
\textbf{Flujo alternativo \texttt{CU-15-FA-01}.} \textit{Condición de disparo:} Uno de los ítems no dispone de la cantidad solicitada antes de confirmar la venta.
\begin{enumerate}[label=\alph*.]
...
\textbf{Precondiciones.}
\begin{itemize}
\item Existe un turno abierto.
\item El usuario está autorizado para ejecutar el cierre.
\end{itemize}
...
\item El sistema compara los valores calculados con el valor declarado.
\item El sistema calcula la diferencia.
\item El sistema registra el cierre, los totales y la diferencia.
\item El sistema confirma el cierre del turno.
\end{enumerate}

\textbf{Flujo alternativo \texttt{CU-16-FA-01}.} \textit{Condición de disparo:} La diferencia calculada es distinta de cero.
\begin{enumerate}[label=\alph*.]
\item El sistema conserva la diferencia como parte del cierre.
\item La interfaz la presenta explícitamente para su seguimiento operativo.
\end{enumerate}
...
\textbf{Flujo alternativo \texttt{CU-17-FA-01}.} \textit{Condición de disparo:} Una novedad existente cambia de responsable, vencimiento o estado.
\begin{enumerate}[label=\alph*.]
...
\textbf{Flujo alternativo \texttt{CU-18-FA-01}.} \textit{Condición de disparo:} Antes de confirmar, el instructor modifica la selección u orden de ejercicios o sus parámetros.
\begin{enumerate}[label=\alph*.]
...
\textbf{Flujo alternativo \texttt{CU-19-FA-01}.} \textit{Condición de disparo:} El ajuste modifica únicamente parámetros de una rutina sin sustituir sus ejercicios.
\begin{enumerate}[label=\alph*.]
